...
**Approvals are not re-checked for withdrawal.** A reviewer can approve a pull request and later request changes, which would make the earlier approval stale. This implementation counts any `APPROVED` review without checking whether it was later superseded. In the sample data this never occurs (no approval was later withdrawn), so the simpler rule gives the same answer as the stricter one would; it is noted as a boundary rather than an observed problem.
